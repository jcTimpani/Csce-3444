...
function renderDropdown(locs) { const sel = document.getElementById("buildingSelect"); sel.innerHTML = '<option value="">Select a building...</option>'; locs.sort((a, b) => a.name.localeCompare(b.name)).forEach(l => { const opt = document.createElement("option"); opt.value = l.id; opt.textContent = l.name; sel.appendChild(opt); }); sel.onchange = (e) => { if (e.target.value) focusLocation(e.target.value); }; }
function filterLocations() { renderMarkers(locationsData); renderDropdown(locationsData); }
